<template>
  <div class="app-container">
    <el-card>
      <template #header>
        <div class="card-header">
          <span>Agno Sessions</span>
          <el-button type="primary" icon="Refresh" circle size="small" @click="loadData" />
        </div>
      </template>

      <div class="filter-bar">
        <el-select v-model="query.type" placeholder="类型" clearable style="width: 130px" @change="handleQuery">
          <el-option label="Agent" value="agent" />
          <el-option label="Team" value="team" />
          <el-option label="Workflow" value="workflow" />
        </el-select>
        <el-input v-model="query.component_id" placeholder="组件 ID" clearable style="width: 200px" @keyup.enter="handleQuery" />
      ...
      <el-table v-loading="loading" :data="tableData" border stripe highlight-current-row>
        ...
        <el-table-column label="Session ID" prop="session_id" min-width="280" show-overflow-tooltip />
        <el-table-column label="名称" prop="session_name" min-width="180" show-overflow-tooltip />
    ...
    <el-dialog v-model="detailVisible" title="Session 详情" width="700px" top="5vh">
...
const query = reactive({
  ...
  component_id: undefined as string | undefined,
...
async function loadData() {
  loading.value = true;
  try {
    const res = await AgnoSessionAPI.listSessions(query);
    const body = res.data;
    tableData.value = body?.data ?? [];
    total.value = body?.meta?.total_count ?? body?.total ?? 0;
  } catch (e) {
...
async function showDetail(row: any) {
  try {
    const res = await AgnoSessionAPI.getSession(row.session_id);
...
async function handleDelete(row: any) {
  await ElMessageBox.confirm(`确认删除会话 ${row.session_id}?`, "警告", { type: "warning" });
  try {
    await AgnoSessionAPI.deleteSession(row.session_id);
